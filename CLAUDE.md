...
- `routes/`: API handlers (auth, channels, videos, videoDetail, videoSearch, channelSearch, config, jobs, plex, setup, subscriptions, apikeys, ytdlpOptions, health, maintenance, playlists, mediaServers, subfolders, jobEvents, thumbnails). All use the dependency injection factory pattern; wiring lives in `server/routes/index.js`.
- `modules/`: class-based singletons holding business logic. Top-level modules include `channelModule`, `downloadModule`, `plexModule`, `jobModule`, `configModule`, `videosModule`, `videoMetadataModule`, `videoSearchModule`, `webSocketServer`, `databaseHealthModule`, `notificationModule`, `channelSettingsModule`, `videoDeletionModule` (nightly auto-removal runs three strategies in order - age, watched, free-space - all excluding protected videos, videos of enabled channels with auto_removal_protected set (channel-level guards go dormant while a channel is unsubscribed), and, when `autoRemovalKeepRecentCount` is set, the N most recently downloaded unprotected videos (cleanup aborts if the guard query fails, and protected videos never consume keep-recent slots); per-channel auto_removal_keep_recent_count guards work the same way (unioned into the exclusion set, fail-closed on guard-query failure) and are mutually exclusive with auto_removal_protected - enabling protection clears the channel's count; the watched strategy honors `watchStatusWatchedRule`, treats never-synced videos as unwatched, and is skipped with a warning when watch status sync is disabled), `autoRemovalQueries` (read-only candidate queries for the watched strategy and the keep-most-recent guards, global and per-channel; candidate queries LEFT JOIN enabled channels to exclude auto_removal_protected channels; watched eligibility SQL comes from `watchStatusQueries.buildWatchedEligibilitySql`, where a qualifying played row with a NULL `last_watched_at` blocks days-since-watched removal), `nfoGenerator`, `cronJobs`, `apiKeyModule`, `ytdlpModule` (yt-dlp version management: `ytdlpUpdateChannel` config selects the stable or nightly release channel, every update runs `--update-to <channel>@latest` so the channel survives container recreation, latest-version checks hit the matching GitHub repo per channel, downloads no longer block updates, and startup re-applies the configured channel via `shouldReapplyChannel` when the installed binary's version shape drifts), `messageEmitter`, `jobAuxData` (serialize/parse for Jobs.aux_data, the JSON snapshot of job.data minus videos - failed downloads, diagnoses, skip counts, terminated channels - written at job save and merged back on startup load so Download History detail survives restarts; persisted failedVideos capped at 1000), `jobEventLog` (append-only video/events log in the `job_events` table: every major step of a job or video - queued, started, download started, file finalized, failed, NZB staged/imported/untracked, deleted, ... - is recorded once via a fire-and-forget `jobEventLog.record(EVENT_TYPES.X, {jobId, youtubeId, detail})` that never throws and never alters the code it observes; `occurred_at` is DATETIME(3) stamped at the call site, `message` is rendered once at write time and stored frozen, rows carry a title/channel/jobType snapshot plus `source` (the job's Download-History-style source label) and `is_tracked` (whether the video was in the library at that moment, or - from job creation on - whether its download job will leave it there; failures record the real state), all captured synchronously inside `record()` from what the caller passes plus in-memory memory (`rememberVideo`/`rememberJob`/`markTracked`, warmed at startup, and fed before a job's events by `download/eventLogVideoPrimer` (names from videos/channelvideos/playlistvideos, library destination) and the post-processor's persisted marker) - nothing is looked up or recomputed on read, so the Event Log page shows stored values only (the sole read-side derivations are display formatting and the gap-since-previous column) - and have no foreign keys so they outlive the Jobs/Videos rows they describe; `runWithContext({ actor, reason }, fn)` tags events recorded inside `fn` (auto-removal, cache expiry); `jest.setup.js` mocks the module globally so no test writes to the DB - assert on `jobEventLog.record` calls; add a new step by adding one entry to `eventCatalog.js` plus one call at the place that does the work; read side is `routes/jobEvents.js` (`GET /api/job-events`, `GET /api/job-events/facets`, and `DELETE /api/job-events` behind Settings > Maintenance & Rescan > Clear Event Log); nightly prune by `jobEventLogRetentionDays`, default 180; this is the intended eventual replacement for the recompute-on-read Download History), `playlistModule` (YouTube playlist lifecycle + auto-create source channels; every fetch pages the full playlist via yt-dlp's default webpage path (full pagination needs yt-dlp >= 2026.07.04), capped at 5000 entries, retrying once via InnerTube (`youtubetab:skip=webpage`, which YouTube caps at ~200 for playlist views) when the default path fails or falls short of the playlist's reported count; auto-download uses seed-then-track: the first auto run per playlist downloads the latest N (legacy position window) and stamps `auto_download_baseline_at`, after which runs select the newest undownloaded rows by first-seen `added_at`, ordering-proof against owner-controlled playlist order; playlist removal is a soft delete (`enabled=false`) that hides the playlist from every id-addressed route, and re-subscribing restores it with its saved settings; every fetch ends with `backfillFromDownloadedVideos`, reconciling rows against already-downloaded videos to fill channel attribution and stamp `added_at` with the real download time; an idle auto-download sweep (>=1 auto-enabled playlist checked, nothing enqueued, no errors) records one synthetic Complete 'Playlist Downloads' job with an empty videos array; like other no-video jobs it only appears in Download History when "Show jobs with no videos" is checked), `m3uGenerator` (writes `__playlists__/*.m3u` fallback files; entries prefer the file matching the playlist's Download Type - mp3 for MP3 Only playlists, video otherwise - and fall back to the other so no downloaded item is dropped; entries follow the playlist's saved `sort_order` ('default' = YouTube playlist order, 'reversed' = flipped); also writes optional per-channel `<channel folder>.m3u` files at the top of each channel directory (per-channel `m3u_enabled`/`m3u_sort_order` settings, entries filtered to files that exist on disk, no `.ignore` marker so Jellyfin/Emby auto-import them; regenerated on download completion, in-app deletion, settings changes, channel enable/disable, and rescan completion (the scheduled nightly rescan doubles as the daily refresh); zero-entry channels get the file deleted instead of an empty playlist)), `playlistDownloadGrouper` (resolves per-video download settings for playlist downloads (override -> channel -> playlist -> global) and groups videos by resolved settings), `subfolderModule` (subfolder registry: `getAll` returns the union of the `subfolders` table + config default + Plex-mapping subfolders; `register` idempotently upserts a name; `delete` guards removal against non-empty disk paths and active references), `channelDownloadAllModule` ("download all videos for a channel tab": selects every known never-downloaded channel video (excluding ignored/YouTube-removed/members-only/live, and anything ever downloaded even if since deleted - download-all is not a re-download mechanism), previews count + total duration for the confirmation dialog, and queues one `doSpecificDownloads` job labelled `Channel Download All: <title>`; these jobs are exempt from the 6-hour absolute runtime cap since large channels legitimately take days, with the 30-minute activity timeout remaining as the hang guard), `manualDownloadGrouper` (buckets manually pasted URLs by resolved per-channel resolution/audio format before job creation; per-URL channel attribution prefers the channelId captured during URL validation (server cache merged over the client echo), then videos.channel_id, then channelvideos listers, gated to tracked+enabled channels; multi-bucket pastes share one downloadRunTracker run labeled 'Manual downloads'; file structure is NOT grouped - manual jobs always download nested to temp and the post-processor resolves flat-vs-subfolder per video from its real channel at finalize, so bulk imports are structurally correct regardless of pre-download attribution), `youtubeUrlParser` (pure YouTube URL -> video-id parsing shared by videoValidationModule and manualDownloadGrouper), `videoThumbnailCache` (every UI video thumbnail is `/images/videothumb-<id>.jpg`, served by `routes/thumbnails.js` mounted ahead of the `/images` static handler (public, like the rest of `/images`): serves the stored file, else fetches YouTube's hqdefault once and keeps it (`?cache=0` = no-cache: stored file if any, else a 302 to YouTube, for channel/playlist listings and search results); a nightly 3:35 AM task deletes kept thumbnails of non-library videos not viewed for 30 days, and purging a video deletes its thumbnail; the client builds every such URL with `utils/videoThumbnail.videoThumbnailUrl(id, { noCache })`), `playlistVideoFilters` (resolves the playlist videos listing's downloadState/watchedState filters into a youtube_id constraint for the paginated page query; models and watchStatusQueries are passed at call time so the playlists route keeps its dependency injection), `channelSearchModule` (YouTube channel search: YouTube Data API search.list type=channel + channels.list enrichment when a key is configured, yt-dlp channel-filtered results-page fallback otherwise; stamps subscribed from enabled Channel rows; videoCount is API-path only), `resolutionTier` (measures a video file's actual pixel dimensions via ffprobe - ground truth over info.json, which can describe a different format than what's on disk - storing the raw `"WIDTHxHEIGHT"` string on `Videos.video_resolution` at download time and via the rescan backfill (`"0x0"` = probe failed, stamped once so failed rows aren't re-probed nightly); also feeds videoMetadataModule's available-resolutions list: format_note for landscape formats, but the selection class (smallest ladder rung >= pixel height, via `selectionTierForHeight`) for vertical ones, because YouTube's format_note labels verticals by short edge (1080x1920 = "1080p") while the app labels them by selection class - without the conversion a top-of-ladder vertical download's tier would be absent from the modal's list. Interpreting dimensions into the displayed tier is deliberately display logic, in `client/src/utils/videoResolution.ts`: landscape maps to the YouTube ladder (wider-than-16:9 by long edge * 9/16, else height, floor-snapped), vertical maps to its SELECTION class (smallest rung >= pixel height, matching the height-capped yt-dlp selector, so 608x1080 reads 1080p and 1080x1920 reads 2160p); the listing chips (`DownloadFormatIndicator`) and the video modal (`608x1080 (1080p)`) share that util, so they always agree, and label-rule changes need no migration or re-probe).
...
- `components/DownloadManager/EventLog/`: the Event Log page at `/downloads/log` (nav item "Event Log"; Download History rows link to a per-job/per-video Timeline). Reads `GET /api/job-events` through `hooks/useJobEvents.ts` and renders only stored columns: numbered pagination and the shared `VideoListContainer` filters (level, event type, source, component, channel, library, dates, search); newest first, oldest first when any filter is active; a row's thumbnail opens the shared `VideoModal`; long messages are cut with a "more…" link that opens the row's expansion; refreshes over WebSocket via `useDownloadListingsRefresh`. The Actor column is shown as "Component" with readable names (`eventLogFormat.componentLabel`).
...
- `contexts/` and `providers/`: React Context for cross-cutting concerns (auth token, WebSocket, theme, PiP playback). `contexts/ThemeEngineContext.tsx` owns the active theme mode, resolves the layout policy for the current viewport, and injects theme CSS variables onto the document root. `contexts/PipPlayerContext.ts` + `providers/PipPlayerProvider.tsx` (mounted above the router in `index.tsx`, so it survives page navigation) expose `play(youtubeId, title)`/`close()`/`state` for the "quick play" Picture-in-Picture icon on each Video Library row (`VideosPage`'s `VideoCard`/`VideosTable`/`VideosListMobile`): `hooks/useHlsPipPlayer.ts` streams `/api/ytstream/:id` (whatever mode is currently configured, including `forceServerSettings`) through `hls.js` when it's an HLS manifest, falling back to a plain `<video src>` otherwise, then calls `requestPictureInPicture()`; if PiP is unavailable/refused it just keeps playing inline in `components/HlsPipPlayer.tsx`'s floating panel instead of failing silently.
...
- **WebSocket server** shares the HTTP server port (3011 in container, 3087 on host) via the `ws` library; there is no separate WS port. Broadcast message types include `downloadProgress`, `downloadComplete`, `jobsUpdated` (emitted when a download job is enqueued or flips to In Progress), and `videosUpdated` (emitted per video mid-batch after its DB rows are persisted); listing pages and the Download History page subscribe via `client/src/hooks/useDownloadListingsRefresh.ts` to refetch in near real time. The server pings clients every 30s and terminates unresponsive ones (proxy idle-timeout guard); on connect it replays only the last stored final-state message, never in-flight progress. The activity page therefore seeds its initial state over REST: `GET /api/jobs/current-activity` returns `downloadModule.getCurrentActivitySnapshot()` (the executor's retained progress monitor read via the side-effect-free `toPayload()`, plus the stored last-final message), consumed by `useCurrentActivitySeed` in `client/src/components/DownloadManager/hooks/`, which seeds only until the first live broadcast arrives and re-probes on tab-visibility and on the provider's local `connectionRestored` message (emitted after every reconnect). `client/src/utils/jobTypeLabel.ts` is the client-side mirror of `server/modules/download/jobTypes.js` labels for the activity header, queued chips, and job summaries.
