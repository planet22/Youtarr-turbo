...
function channelVideoToModalData(
  ...
  return {
    ...
    isStrm: Boolean((video as { is_strm?: boolean; filePath?: string }).is_strm) ||
      (typeof (video as { filePath?: string }).filePath === 'string' &&
        (video as { filePath?: string }).filePath!.toLowerCase().endsWith('.strm')),
...
function ChannelVideos({
  ...
  // Filters, and (TV Series mode) the season/episode it would resolve to.
  const [applyChannelFilters, setApplyChannelFilters] = useState(false);
  ...
  const hasChannelMediaModeOverride = Boolean(channelMediaMode);
  const defaultMediaMode = channelMediaMode || config.mediaMode || 'download';
  const defaultMediaModeSource: 'channel' | 'global' = hasChannelMediaModeOverride ? 'channel' : 'global';
  ...
  // Not a content filter (nothing is hidden/excluded) - a display toggle
  // that asks the server to attach each row's channelFilterPreview (see
  // Status column rendering below), so passed via customFilters rather
  // than filterConfigs/ToggleChipFilter's off/only/exclude machinery.
  const channelFilterPreviewToggle = (
    <Tooltip title="Show whether each video would pass this channel's own Download Filters (Duration/Title/Season-Episode), without changing which videos are listed">
      <Chip
        icon={<ChannelFilterPreviewIcon size={16} />}
        label="Preview channel filters"
        variant={applyChannelFilters ? 'filled' : 'outlined'}
        color={applyChannelFilters ? 'primary' : 'default'}
        size="small"
        onClick={() => setApplyChannelFilters((prev) => !prev)}
        style={{ cursor: 'pointer' }}
        aria-pressed={applyChannelFilters}
      />
    </Tooltip>
  );
