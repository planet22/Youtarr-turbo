...
interface DownloadAllVideosDialogProps {
  ...
  // Effective media_mode ('download'/'strm'/'both') this run will actually
  // use — channel override if set, else the global default — and where it
  // came from, purely for the confirm-step copy below.
  defaultMediaMode?: string;
  defaultMediaModeSource?: 'channel' | 'global';
...
function DownloadAllVideosDialog({
  ...
  defaultMediaMode = 'download',
  defaultMediaModeSource = 'global',
  runMetadataFetch,
  onStarted,
  ...
  const isBoth = defaultMediaMode === 'both';
  const mediaModeLabel = MEDIA_MODE_LABEL[defaultMediaMode] || defaultMediaMode;
  const mediaModeSourceLabel = defaultMediaModeSource === 'channel' ? 'channel setting' : 'global default';
      ...
      >
        ...
        <DialogContent>
          <div style={{ paddingTop: 8 }}>
            ...
            {!preparing && preview && preview.count > 0 && (
              <>
                <Alert severity={isStrmOnly ? 'info' : 'warning'} style={{ marginBottom: 16 }}>
                  <Typography variant="body2">
                    {isStrmOnly ? (
                      <>
                        This will create {preview.count} STRM pointer file{preview.count === 1 ? '' : 's'} for
                        on-demand streaming{contentDuration ? ` (${contentDuration})` : ''} — no video files
                        will be downloaded, so this should be quick.
                      </>
                    ) : (
                      <>
                        You are about to download {preview.count} video{preview.count === 1 ? '' : 's'}
                        ...
                        {isBoth ? ' STRM pointer files will also be created alongside the downloads.' : ''}
                      </>
                    )}
                  </Typography>
                </Alert>

                <Typography variant="caption" color="text.secondary" style={{ display: 'block', marginBottom: 16 }}>
                  Media mode: {mediaModeLabel} ({mediaModeSourceLabel})
                </Typography>
