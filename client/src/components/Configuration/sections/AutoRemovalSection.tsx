...
export const AutoRemovalSection: React.FC<AutoRemovalSectionProps> = ({
  ...
  const handleMinFileSizeChange = (rawValue: string) => {
    const parsed = parseInt(rawValue, 10);
    onConfigChange({
      autoRemovalMinFileSizeKB: Number.isNaN(parsed) || parsed < 0 ? 0 : parsed
    });
  };
                  ...
                  <Grid item xs={12} md={6}>
                    <Box className="flex items-center">
                      <TextField
                        fullWidth
                        type="number"
                        label="Never remove files smaller than (KB)"
                        value={config.autoRemovalMinFileSizeKB > 0 ? String(config.autoRemovalMinFileSizeKB) : ''}
                  ...
                  <Grid item xs={12}>
                    <Box className="flex items-center">
                      <FormControlLabel
                        control={
                          <Switch
                            checked={config.autoRemovalPreserveStrmFallback !== false}
                            onChange={(e) => onConfigChange({ autoRemovalPreserveStrmFallback: e.target.checked })}
                          />
                        }
