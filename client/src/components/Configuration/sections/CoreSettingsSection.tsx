      ...
      <Grid container spacing={2} className="mt-2">
        ...
        <Grid item xs={12}>
          <Accordion defaultExpanded style={{ border: 'var(--border-weight) solid var(--border)', borderRadius: 'var(--radius-ui)' }}>
            <AccordionSummary>
              <Typography variant="subtitle2" style={{ fontWeight: 700 }}>
                TV Series File Structure Settings
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth>
                    <InputLabel id="default-library-mode-label">Default Library Mode</InputLabel>
                    <Select
                      labelId="default-library-mode-label"
                      value={config.defaultLibraryMode}
                      onChange={(e: SelectChangeEvent<string>) => onConfigChange({ defaultLibraryMode: e.target.value as ConfigState['defaultLibraryMode'] })}
                      label="Default Library Mode"
                    >
                ...
                <Grid item xs={12} md={6}>
                  <Box className="flex items-start">
                    <SubfolderAutocomplete
                      mode="global"
                      value={config.seriesOutputSubfolder || null}
                      onChange={(value) => onConfigChange({ seriesOutputSubfolder: value || '' })}
                      subfolders={subfolders}
                      loading={subfoldersLoading}
                      createSubfolder={createSubfolder}
                      label="TV Series Output Subfolder"
                      helperText="Where series-mode channels land by default. Leave empty to use the same location as movie mode."
                    />
                    <Box className="flex items-center min-h-[48px] mt-5">
                      <InfoTooltip
                        text="Applies only to channels/playlists using TV Series library mode, and only when that channel has no subfolder of its own already set. Point a separate Jellyfin 'Shows' library at this subfolder to keep TV Series content separate from Movies. Only affects new downloads."
                        onMobileClick={onMobileTooltipClick}
                      />
                    </Box>
                  </Box>
                ...
                <Grid item xs={12}>
                  <FormControl fullWidth>
                    <InputLabel htmlFor="episodeFilenamePrefix">Episode Filename Template</InputLabel>
                    <TextField
                      id="episodeFilenamePrefix"
                      fullWidth
                      name="episodeFilenamePrefix"
                      value={config.episodeFilenamePrefix}
