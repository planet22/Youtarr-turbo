...
export const AutoRemovalRulesSummary: React.FC<AutoRemovalRulesSummaryProps> = ({ config }) => {
  ...
  const keptExtras: React.ReactNode[] = [];
  if (config.autoRemovalKeepRecentCount > 0) {
    keptExtras.push(
      <>the <strong>{config.autoRemovalKeepRecentCount}</strong> newest downloads</>
    );
  }
  if (config.autoRemovalMinFileSizeKB > 0) {
    keptExtras.push(
      <>files smaller than <strong>{config.autoRemovalMinFileSizeKB} KB</strong></>
    );
  }
