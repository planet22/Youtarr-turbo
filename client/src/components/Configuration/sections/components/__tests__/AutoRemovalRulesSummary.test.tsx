...
describe('AutoRemovalRulesSummary', () => {
  ...
  test('includes the minimum file size in the kept line when set', () => {
    renderWithProviders(
      <AutoRemovalRulesSummary
        config={createConfig({
          autoRemovalVideoAgeThreshold: '30',
          autoRemovalMinFileSizeKB: 1,
        })}
      />
    );

    expect(screen.getByText(/Always kept:/i)).toHaveTextContent(
      'Always kept: Protected videos, and files smaller than 1 KB.'
    );
  });

  test('joins keep-recent count and minimum file size with a final and', () => {
    renderWithProviders(
      <AutoRemovalRulesSummary
        config={createConfig({
          autoRemovalVideoAgeThreshold: '30',
    ...
    expect(screen.getByText(/Always kept:/i)).toHaveTextContent(
      'Always kept: Protected videos, the 50 newest downloads, and files smaller than 1 KB.'
    );
  });
});
