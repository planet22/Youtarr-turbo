...
const DownloadProgress: React.FC<DownloadProgressProps> = ({
  ...
  const overlayContent = useMemo(() => {
    ...
    const showEta =
      ...
      currentProgress.state === 'transcoding';
  ...
  const statusMessage = useMemo(() => {
    ...
    switch(currentProgress.state) {
      ...
      case 'materializing_strm': return 'Materializing STRM files...';
      case 'transcoding': return 'Transcoding downloaded video...';
        ...
        {currentProgress && showProgress && (
          <Box className="px-4 pb-4">
            ...
            <Box className="flex justify-between items-center mt-1">
              ...
              {/* Transcoding reuses speedBytesPerSecond to carry ffmpeg's own
                  realtime speed factor (e.g. 1.5 for "1.5x realtime"), not an
                  actual byte rate - format accordingly. */}
              {currentProgress.progress && currentProgress.state === 'transcoding' && (
                <Typography variant="caption" color="text.secondary">
                  {(currentProgress.progress?.speedBytesPerSecond ?? 0).toFixed(2)}x speed •{' '}
                  {(currentProgress.progress?.percent ?? 0).toFixed(1)}%
                </Typography>
              )}
            </Box>
