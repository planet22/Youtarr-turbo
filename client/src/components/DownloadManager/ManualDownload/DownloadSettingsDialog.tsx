...
interface DownloadSettingsDialogProps {
  ...
  // Only pass this when the whole batch is known to share one media mode
  // (e.g. a single channel/playlist's own setting) - omit it for contexts
  // that can span multiple channels with different modes (the Videos
  // library page, "Manually Added Urls"), where "STRM only" can't be
  // assumed and the generic disk-space warning below stays accurate.
      ...
      <DialogContent>
        <Box className="pt-2">
          ...
          {typeof videoCount === 'number' && videoCount > LARGE_DOWNLOAD_WARNING_THRESHOLD && (
            <Alert severity={isStrmOnly ? 'info' : 'warning'} className="mb-4">
              <Typography variant="body2">
                {isStrmOnly ? (
                  <>
                    That is a large batch ({videoCount} videos), but this creates STRM pointer
                    files for on-demand streaming — no video files will be downloaded, so this
                    should be quick and use negligible disk space.
                  </>
                ) : (
                  <>
