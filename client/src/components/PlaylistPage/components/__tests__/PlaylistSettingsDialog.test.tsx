...
describe('PlaylistSettingsDialog', () => {
  ...
  test('persists an edited download type', async () => {
    ...
    expect(
      screen.getByText(
        'MP3 files are saved at 192kbps in the same folder as videos. MP3 Only playlists sync to media servers as music playlists: the server needs a music-type library that includes your Youtarr-Turbo output folder.'
      )
    ).toBeInTheDocument();
