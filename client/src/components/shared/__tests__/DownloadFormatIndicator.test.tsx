...
describe('DownloadFormatIndicator', () => {
  describe('Rendering', () => {
    ...
    test('shows a STRM chip instead of the size for a .strm video file', () => {
      render(<DownloadFormatIndicator filePath="/videos/test.strm" fileSize={0} />);

      expect(screen.getByText('STRM')).toBeInTheDocument();
      expect(screen.queryByText('0MB')).not.toBeInTheDocument();
      expect(screen.getByTestId('StrmFormatIcon')).toBeInTheDocument();
      expect(screen.queryByTestId('VideoFormatIcon')).not.toBeInTheDocument();
    });

    test('shows a STRM chip instead of the size for a .strm audio file', () => {
      render(<DownloadFormatIndicator audioFilePath="/audio/test.strm" audioFileSize={0} />);

      expect(screen.getByText('STRM')).toBeInTheDocument();
      expect(screen.getByTestId('StrmFormatIcon')).toBeInTheDocument();
      expect(screen.queryByTestId('AudioFormatIcon')).not.toBeInTheDocument();
    });

    test('matches .strm case-insensitively', () => {
      render(<DownloadFormatIndicator filePath="/videos/test.STRM" fileSize={0} />);
