# Youtarr-Turbo Setup Guide

## Prerequisites

Before setting up Youtarr-Turbo, ensure you have:
...
> Running on a NAS or Unraid? There are dedicated platform guides for [Synology](platforms/synology.md) and [Unraid](platforms/unraid.md) - start there instead.
...
2. **Start Youtarr-Turbo**:
   ...
   #### Optional flags:
     - `--no-auth`: Completely disable auth. Never expose Youtarr-Turbo directly to the internet in this manner, only use if you have your own authentication layer (Cloudflare Tunnel, OAuth Proxy, etc)
...
3. **Edit the .env file**:
   ...
   - `TRUST_PROXY=false` - Recommended when exposing Youtarr-Turbo directly without a reverse proxy. Leave unset for the current backwards-compatible default.
...
- If you need to reset your admin password, see the [Troubleshooting Guide](TROUBLESHOOTING.md#forgotten-admin-password-reset-admin-password)

## Configuration

After logging in, configure Youtarr-Turbo through the Settings page.
...
- After changing the download directory, you must restart Youtarr-Turbo (`./stop.sh` then `./start.sh` or `docker compose down` and `docker compose up -d`)
...
The `config/complete.list` file tracks all downloaded videos and prevents re-downloads during automatic channel updates. **Deleting this file will cause Youtarr-Turbo to re-download ALL videos from all subscribed channels on the next scheduled run.** This file also stores videos you've marked to ignore for automatic downloads.
...
Videos must retain their `[youtubeid].mp4` filename and remain in the Youtarr-Turbo configured mount. Moving or renaming files will cause Youtarr-Turbo to mark them as "missing" from disk.
If videos are moved WITHIN the mount, on restart, Youtarr-Turbo will attempt to find them, but do so at your own risk.
...
**Network Storage**: Youtarr-Turbo supports NAS, network shares, and mounted volumes. Ensure:
- Youtarr-Turbo container can write to the media location (local or network storage)
- Your media server (Plex/Jellyfin/etc.) can read from the same media location
- Youtarr-Turbo can reach your media server API over the network (if using Plex integration)
...
**Automatic Filtering**: Youtarr-Turbo automatically skips subscriber-only content. You can configure auto-downloads separately for long-form videos, Shorts, and Streams in each channel's settings.
...
Youtarr-Turbo fully supports platform-managed deployments with automatic configuration:
...
To access Youtarr-Turbo from other devices on your private network:

1. Configure your firewall to allow port 3087
2. Access using your server's IP address: `http://[server-ip]:3087`

For external access:
- Do not expose Youtarr-Turbo directly to the internet over plain HTTP
- Use a reverse proxy with HTTPS, or use a VPN/SSH tunnel instead of port forwarding the app directly
- If you use a reverse proxy, make sure WebSocket support is enabled for the Youtarr-Turbo host - otherwise real-time download progress won't display. See [Troubleshooting](TROUBLESHOOTING.md#no-download-progress-shown-downloads-work-videos-just-appear)
- Keep `AUTH_ENABLED=true` unless an upstream authentication layer protects every request

## Upgrading

### Important: Youtarr-Turbo Does Not Auto-Update

**Youtarr-Turbo does not automatically update itself.** When you run `./start.sh` or `docker compose up -d`, it uses your currently installed version. Simply restarting Youtarr-Turbo without pulling updates will restart the same version you currently have installed.
...
- **Check your current version**: Look in the footer of the Youtarr-Turbo web interface
- **Compare versions**: If your version number is older than the latest release, an update is available

### How to Update to the Latest Version

Choose the method that matches how you installed Youtarr-Turbo:
...
3. Restarts Youtarr-Turbo with the new version
