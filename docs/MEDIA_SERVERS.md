...
Youtarr-Turbo supports integration with multiple media servers, providing rich metadata and organized content for your YouTube video collection.
...
|---------|------|------|----------|------|
| Automatic Library Refresh via Youtarr-Turbo | ✅ | ❌ | ❌ | ❌ |
| Native Playlist Sync | ✅ | M3U only | ✅ | ✅ |
| Channel Playlist (.m3u) Auto-Import | ❌ | Manual import | Mixed or Music libraries only | Mixed-type libraries only |
| Watch Status Sync (into Youtarr-Turbo) | ✅ | ❌ | ✅ | ✅ |
| NFO Metadata | ❌ | ✅ | ✅ | ✅ |
| Embedded MP4 Metadata | ✅ | Partial | ✅ | ✅ |
| Channel Posters | ❌ | ✅ | ✅ | ✅ |
| Backdrop Images | ❌ | ❌ | ✅ | ✅ |

### Metadata Generated by Youtarr-Turbo
...
Youtarr-Turbo supports organizing content into separate libraries using subfolders:
...
- For native playlist sync, you do need to connect the server in Youtarr-Turbo: enable it under Settings, then enter the server URL, an API key, and the user account that should own the playlists. See [Media Server Playlists](MEDIA_SERVER_PLAYLISTS.md) for the full setup.

#### Kodi
- No specific configuration needed for metadata; point Kodi at the download directory.
- Kodi has no native playlist sync, but Youtarr-Turbo writes a `.m3u` file you can import. See [Media Server Playlists](MEDIA_SERVER_PLAYLISTS.md#m3u-fallback).
...
| `<dateadded>`        | Date the video was downloaded by Youtarr-Turbo  |
| `<studio>`           | Channel name                              |
...
| `<mpaa>` / `<ratings>` | Content rating when one applies (download override, channel/playlist default, or YouTube age rating) |
| `<trailer>`          | Link back to the video on YouTube         |
...
Youtarr-Turbo can mirror subscribed YouTube playlists into Plex, Jellyfin, and Emby as native playlists, plus emit a universal `.m3u` fallback. Per-server setup, visibility model, and replace semantics are documented separately in [Media Server Playlists](MEDIA_SERVER_PLAYLISTS.md).

## Watch Status Sync

Youtarr-Turbo can pull per-video, per-user watch status from Plex, Jellyfin, and Emby, show a Watched chip on its listing pages, and let you filter your library by watched state. The sync is one-way; nothing is written back to your servers. See [Track Watch Status from Media Servers](USAGE_GUIDE.md#track-watch-status-from-media-servers) for the workflow and the [Configuration Reference](CONFIG.md#watch-status-sync) for the config keys.
...
2. **Enable Metadata Generation** in Youtarr-Turbo (Settings -> Core)

3. **Add Channels** and let Youtarr-Turbo download with metadata
