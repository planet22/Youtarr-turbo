...
Youtarr-Turbo can mirror your subscribed YouTube playlists to Plex, Jellyfin, and Emby as native playlists, plus generate a universal `.m3u` fallback file. This document covers per-server setup, visibility models, and the M3U fallback.

## Overview

When you subscribe to a YouTube playlist:

1. Youtarr-Turbo fetches the playlist metadata and video list from YouTube via `yt-dlp`.
...
3. After each download (and after manual downloads of videos that happen to live in a tracked playlist), Youtarr-Turbo syncs the playlist to every configured media server and regenerates the M3U file.
...
What Youtarr-Turbo does **not** do for playlists:
...
- Uses a single configured account per Jellyfin/Emby server. A playlist is either private to that account or public to everyone on the server (set the **Public on media servers** toggle); Youtarr-Turbo can't target a playlist to a specific subset of other users.

## Playlist order

By default, Youtarr-Turbo keeps the YouTube playlist's own order everywhere it publishes the playlist: the native server playlists and the `.m3u` file. That order is controlled by the playlist's owner on YouTube.

Some playlists add new videos at the top, so they play newest-first even when they are meant to be watched from the beginning; think of a series with an overarching story, or a course. For those, set **Playlist order** to **Reverse playlist order** on the playlist page (under **Playlist settings**). Youtarr-Turbo then writes the `.m3u` and syncs the server playlists oldest-first, and the order stays correct as the owner adds new videos.
...
- **Playlist visibility scope (advanced)**: under Settings -> Plex -> "Advanced: playlist visibility scope", a selector controls which account owns Youtarr-Turbo playlists. It maps to the `plexPlaylistToken` config value:
...
- **Visibility / sharing**: on a claimed server, Plex playlists are created under your admin account and are visible only to you. To share one with another user, open the playlist in Plex Web (playlist menu -> Share -> pick a user) or use Settings -> Manage Library Access -> [user] -> Media. Youtarr-Turbo cannot grant per-user access programmatically. Note that shared playlists do **not** show up in the recipient's "Playlists" section - Plex lists them under a separate sidebar source named **Media** (see [Troubleshooting](#troubleshooting)).
...
1. In Jellyfin, go to **Dashboard -> API Keys** and create a new API key for Youtarr-Turbo.
2. Find your User ID under **Dashboard -> Users** (it is a GUID in the URL when editing a user), or open the **Jellyfin User** dropdown in Youtarr-Turbo's Jellyfin settings to load the accounts from your server and pick yours.
3. (Optional) Leave **Video Library IDs** blank. You can enter a comma-separated list of library IDs that hold your Youtarr-Turbo videos, but Youtarr-Turbo already matches downloaded videos to Jellyfin items across all your libraries, so this is rarely needed.
...
3. (Optional) Leave **Video Library IDs** blank; Youtarr-Turbo matches videos across all your libraries.
4. **Test Connection**, then toggle **Enable Emby integration**.

Note: Emby's `POST /Playlists` accepts query params with comma-separated `Ids`; Jellyfin's accepts a JSON body with an array. Youtarr-Turbo's adapter handles both, so you don't have to do anything special.

**Visibility**: a **Public** Youtarr-Turbo playlist is created as a server-wide (shared) Emby playlist that all users can see; a **Private** one is owned by the configured user account only. Emby fixes this when the playlist is created, so flipping Public/Private on a playlist that already exists only takes effect on the next sync that recreates it (see [Replace semantics](#replace-semantics) below).

One thing to expect with Public playlists: Emby shows them as read-only and hides the per-user visibility control. That is normal. Youtarr-Turbo owns these playlists and rewrites them on every sync, so any manual edits would be overwritten anyway.
...
- **Plex**: create a **Music** library pointing at your Youtarr-Turbo output folder. A video library and a music library can point at the same folder; each scanner only indexes its own media type. Channel folders will show up as "artists" in the music library, which is cosmetic.
...
Removing a playlist in Youtarr-Turbo (trashcan icon on the Channels & Playlists page) is a soft delete: it stops auto-downloads, hides the playlist from the UI and API, and stops future syncs. It does **not** delete downloaded videos, does not remove the playlist from your media server (delete it there directly if you want it gone), and does not delete the `.m3u` file. Re-adding the same playlist later restores it with its previous settings and download history intact.

## M3U fallback

For every tracked playlist, Youtarr-Turbo writes a `.m3u` file under:

```
<youtubeOutputDirectory>/__playlists__/<sanitized playlist title>.m3u
```

The M3U uses **relative paths** so the file remains valid when the storage volume is mounted at a different host path than where Youtarr-Turbo writes it. Open it in any media player that understands M3U:
...
Note that Youtarr-Turbo writes a `.ignore` marker file inside `__playlists__`, so Jellyfin and Emby skip the folder during library scans and never auto-import these files (unlike the per-channel `.m3u` files, which have no marker). Importing them into those servers is a manual step.
