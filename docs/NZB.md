...
Youtarr-Turbo can act as both a Newznab-compatible search indexer and a SABnzbd-compatible
download client, so Sonarr, Radarr, and Prowlarr can search YouTube through Youtarr-Turbo
and "grab" a result to trigger a real Youtarr-Turbo download (or `.strm` materialization)
...
at `/nzb` (not `/api`, so it doesn't inherit Youtarr-Turbo's own rate limiter/session auth).
Search is powered by Youtarr-Turbo's existing `videoSearchModule` (YouTube Data API with an
...
2. Add at least one **Category**. Each category is what Sonarr/Radarr will show as
   ...
   - **Subfolder** — where grabbed videos land (a subfolder under Youtarr-Turbo's existing
     output root, same as any other Youtarr-Turbo subfolder).
...
This is the one thing that must be set up outside Youtarr-Turbo's own config. When a grab
completes, Youtarr-Turbo reports the real file's path back to Sonarr/Radarr via the SABnzbd
...
read that path.** If Youtarr-Turbo and Sonarr/Radarr run in separate containers (the normal
case), bind-mount the category's output folder (or Youtarr-Turbo's whole output directory)
...
correctly in Youtarr-Turbo, but Sonarr/Radarr will fail to import them.
...
- **Episode-mode search is best-effort, not exact.** YouTube videos have no real
  ...
  episode number — the same heuristic Youtarr-Turbo's own series library mode uses at
