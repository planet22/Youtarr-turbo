...
|--------|------|------|---------|
| `GET` | `/api/ytstream/:youtubeId` | Session token **or** `key` query param | Resolve + play. Query params below. A logged-in browser (video modal, PiP preview) sends its session token; every other caller (media servers reading a `.strm` file, StrmToolTurbo, curl) has no session, so `strmGenerator.js` bakes the per-installation `ytstream.streamKey` into the URL as `?key=...` instead — see "Stream key" below. `AUTH_ENABLED=false` skips this check entirely, same as the rest of the app. |
...
| `quality` | e.g. `720`, `1080`, `best`, or any height | `ytstream.quality` config, else `preferredResolution`, else `720` | `mode=direct`/`direct-redirect` map to the plugin's progressive playback targets (`720` → BroadCompatibility, `1080` → Balanced1080p, `best` → MaximumQuality), capped at whatever progressive (already-muxed) format YouTube actually serves — in practice that's ~360p today; YouTube stopped publishing progressive formats above it years ago. `hls`/`hls-buffer` instead fetch a **DASH** video-only + audio-only pair, so they aren't capped at progressive's ceiling and genuinely support `1080`/`1440`/`2160`. See `qualityStrictness` for how a mismatch with what's actually available is handled. |
...
| `key` | the current `ytstream.streamKey` | — | Required in place of a session token for any caller with no login (media servers, StrmToolTurbo, curl) — see "Stream key" below. Compared with `crypto.timingSafeEqual`; a missing/wrong value gets `403` before any yt-dlp/ffmpeg work starts. Not needed when a valid session token (`x-access-token` header or `token` query param) is present instead. |
| `token` | a valid session token | — | Alternative to `key` for a logged-in caller with no way to set a header (e.g. a plain `<video src>` URL) — same session token the rest of the app uses. |

All of the query params above are ignored entirely when `ytstream.forceServerSettings` is `true` — every request then uses the current `ytstream` config as-is, regardless of what's in the request URL or an already-written `.strm` file. Off by default. `key`/`token` are the exception: they're an auth gate, not a playback setting, and are always checked regardless of `forceServerSettings`.

### Stream key

`GET /api/ytstream/:youtubeId` has no login page of its own to sit behind — a media server reads a `.strm` file as a bare URL, with no way to attach a session header — so without something in the URL itself, anyone who can reach the port could hand this route any YouTube ID and make it run yt-dlp/ffmpeg on the server's behalf (and, in `hls-buffer`, write the whole video into the untracked cache) for content that was never actually downloaded or tracked. `ytstream.streamKey` (see `docs/CONFIG.md`) is a per-installation secret generated once on first run; `strmGenerator.js` bakes it into every generated `.strm` file's URL as `?key=...`. The in-app browser preview (video modal, Picture-in-Picture) uses its own session token instead of the key, since it's already authenticated.

If the key ever leaks (an exposed reverse proxy, a shared `.strm` file), rotate it from Settings → Maintenance → **Stream key**. Rotating immediately invalidates every `.strm` file already on disk; the same action kicks off a metadata regeneration sweep that rewrites `.strm` files for currently-tracked videos automatically. Anything no longer in the library (already removed) is not reachable by that sweep and is left with a broken, unrecoverable-by-rotation `.strm` file.
...
- **MKV** (`container=mkv`): `-f matroska` written directly (no HLS muxer/playlist). The real duration is written into the Matroska Segment Info (`Duration`, or ffmpeg's reserved 11-byte Void slot), the final size is declared up front from yt-dlp's stream sizes (0.5% margin, measured overhead is ~0), and the leftover is padded with an EBML Void element when the encode ends. Reads past the written bytes wait for the encode to catch up, so forward seeks work. The mkv cache key differs from the mp4 one and its cache files are `.mkv` (older ones named `.mp4` are still found).
  ...
  **MKV resume** (`byteRangeResumeCache`): a partial `.mkv` is extended by byte splicing, with no ffmpeg/ffprobe step. The partial is scanned for its complete clusters; the cut is the last cluster that starts on a video keyframe at least 12 s before its end. The base is served at once while yt-dlp downloads the streams in full at normal speed (a sectioned `--download-sections` download was tried first and ran at ~0.5 MB/s, about twice real time, because YouTube throttles ffmpeg's own HTTP client) and ffmpeg runs exactly as it did for the fresh encode that made the cached part (same timeline and cluster boundaries; trimming with `-ss`/`-copyts` was tried and did not line up), so the clusters before the seam are simply skipped when joining; the resume pass's own header, its overlap with the cached part, and its Cues are left out. The seam is checked: yt-dlp's section download can start several seconds BEFORE the requested time (on a keyframe or fragment boundary), so the resume pass usually repeats a little of what the cached part already holds. The resume clusters before the seam are skipped and the join is made at the resume cluster that starts on a keyframe at the cut point (within 100 ms). If the resume pass starts AFTER the cut, or passes it without a cluster starting there, the resume is abandoned, the partial is flagged `mkvResumeFailed` in its sidecar and the next play does a fresh encode. The spliced result has no Cues (seeking works the same way as on a growing file). Debug lines: "mkv resume - scanned the cached partial", "mkv resume seam verified", "mkv resume spliced".
...
**Changing mode on a library that already has `.strm` files** (typically while
experimenting): each `.strm` has a `.strmtool.json` sidecar declaring the
container (`hls` for `hls`/`hls-buffer`/`youtube-hls`/manifest byte-range, `mp4`
or `mkv` for Byte-range Plain file, `mp4` for `download-cache`), and the
StrmToolTurbo Jellyfin plugin reads it instead of probing the stream. Once the
mode changes, regenerate the sidecars (Settings → Maintenance & Rescan →
Regenerate video metadata, or `POST /api/maintenance/regenerate-metadata`), then
run the plugin's extraction from Settings → Jellyfin with **Force refresh:
ignore existing media streams** on. Steps and reasoning are in
[GETTING_STARTED_STREAMING.md](GETTING_STARTED_STREAMING.md#switching-modes-on-an-existing-library).
...
**AMF on the published Docker image**: Debian's stock `ffmpeg` package is
not built with `h264_amf` support (confirmed via `ffmpeg -encoders` on the
published image — `nvenc`, `qsv`, and `vaapi` are present, `h264_amf` is
not). `hardwareMode=amf` will report "unsupported" in the Hardware
Capabilities test on an unmodified image regardless of GPU/driver setup;
using it requires a custom `ffmpeg` build with AMF compiled in.
