...
describe('server routes - version', () => {
  test('GET /getCurrentReleaseVersion returns GitHub release version', async () => {
    const { app } = await createServerModule();
  ...
  test('GET /getCurrentReleaseVersion returns null version when no releases published', async () => {
    const { app, httpsMock } = await createServerModule();

    httpsMock.get.mockImplementationOnce((url, options, callback) => {
      const cb = typeof options === 'function' ? options : callback;
      const mockResp = {
        statusCode: 404,
        on: jest.fn((event, handler) => {
