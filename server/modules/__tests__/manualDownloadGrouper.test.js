...
test('one global-settings group when nothing is attributable', async () => {
  ...
  expect(groups).toEqual([
    { resolution: '1080', audioFormat: null, mediaMode: 'download', urls: [URL_A, URL_B] },
  ]);
});
...
test('applies channel command settings via the attribution map', async () => {
  ...
  expect(groups).toEqual([
    { resolution: '720', audioFormat: null, mediaMode: 'download', urls: [URL_A] },
  ]);
});
...
test('falls back to videos.channel_id, then channelvideos listers', async () => {
  ...
  expect(groups).toContainEqual({ resolution: '720', audioFormat: null, mediaMode: 'download', urls: [URL_A] });
  expect(groups).toContainEqual({ resolution: '1080', audioFormat: 'mp3_only', mediaMode: 'download', urls: [URL_B] });
});
...
test('dialog overrides pin command settings into a single group', async () => {
  ...
  expect(groups).toEqual([
    { resolution: '480', audioFormat: null, mediaMode: 'download', urls: [URL_A, URL_B] },
  ]);
});
...
test('an unparseable URL keeps its original string and joins the global group', async () => {
  ...
  expect(groups).toEqual([
    { resolution: '1080', audioFormat: null, mediaMode: 'download', urls: [weird] },
  ]);
});
