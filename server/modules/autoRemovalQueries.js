...
class AutoRemovalQueries {
   ...
   */
  async getWatchedRemovalCandidates({ minDaysSinceWatched = 0, minVideoAgeDays = 0, excludeIds = [], minFileSizeBytes = 0 } = {}) {
    const { Sequelize, sequelize } = require('../db.js');
...
`;
      ...
      if (minFileSizeBytes > 0) {
        minSizeClause = '          AND (Videos.fileSize IS NULL OR Videos.fileSize >= :minFileSizeBytes)\n';
      ...
      const query = `
        ...
        WHERE Videos.removed = 0
          ...
          AND COALESCE(ProtChannel.auto_removal_protected, 0) = 0
          AND ${watched.sql}
${excludeClause}${minSizeClause}        GROUP BY Videos.id
