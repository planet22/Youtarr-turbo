...
class AutoDownloadScheduler {
  ...
  async getEnabledChannelDownloadUrls() {
    const channels = await Channel.findAll({
      where: { enabled: true },
      attributes: ['channel_id', 'url', 'auto_download_enabled_tabs', 'media_mode']
    });

    const urls = [];
    for (const channel of channels) {
      // STRM-tier channels don't go through this batch yt-dlp URL path at
      // all — they're materialized separately by channelStrmDownloader
      // (see DownloadModule.doChannelDownloads). Excluding them here, at
      // the source, keeps every caller of this function (the pre-flight
      // empty-check, generateChannelsFile(), and the single-job fast path)
      // consistent without each needing its own STRM awareness.
      if (downloadSettingsResolver.resolveMediaMode({ channel, config: configModule.config }) === 'strm') {
        continue;
      }
