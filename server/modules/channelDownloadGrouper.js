...
class ChannelDownloadGrouper {
  ...
  /**
   * Enabled channels whose effective media_mode is 'strm'. These are
   * excluded from the batch yt-dlp grouping/download path (see
   * generateDownloadGroups) and handled instead by channelStrmDownloader,
   * which materializes .strm files from individually-resolved candidate
   * video URLs rather than a batch channel-tab yt-dlp invocation.
   *
   * 'both' is intentionally NOT treated as STRM-tier here: elsewhere in the
   * app (doSpecificDownloads' STRM early-exit) 'both' still falls through to
   * the normal download path today, so excluding only 'strm' keeps this
   * consistent instead of inventing new semantics for 'both'.
   */
  async getStrmModeChannels(config = configModule.config) {
    const channels = await this.getEnabledChannelsWithSettings();
    return channels.filter(
      (channel) => downloadSettingsResolver.resolveMediaMode({ channel, config }) === 'strm'
    );
  }
  ...
  async generateDownloadGroups(overrideQuality = null) {
    const allChannels = await this.getEnabledChannelsWithSettings();
    // STRM-tier channels don't go through the batch yt-dlp/temp-channels-file
    // path at all — see getStrmModeChannels / channelStrmDownloader.
    const channels = allChannels.filter(
      (channel) => downloadSettingsResolver.resolveMediaMode({ channel, config: configModule.config }) !== 'strm'
    );
