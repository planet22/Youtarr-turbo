...
class ConfigModule extends EventEmitter {
  constructor() {
    ...
    // Handle legacy field name (ytstream.fakeLength → ytstream.calculatedLength)
    // This is a one-time migration for old configs
    if (this.config.ytstream && this.config.ytstream.fakeLength !== undefined && this.config.ytstream.calculatedLength === undefined) {
      this.config.ytstream.calculatedLength = this.config.ytstream.fakeLength;
      delete this.config.ytstream.fakeLength;
      legacyMigrationNeeded = true;
      logger.info('Migrated legacy ytstream.fakeLength field to ytstream.calculatedLength');
    }

    // Handle legacy field name (nzb.categories[].newznabCategoryId (single
    // string) → newznabCategoryIds (array)) - lets one Youtarr category
    // match a search naming any of several Newznab category ids instead of
    // just one (see findCategory in server/routes/nzb.js).
    if (Array.isArray(this.config.nzb?.categories)) {
      let categoryIdsMigrated = false;
      for (const category of this.config.nzb.categories) {
        if (category.newznabCategoryId !== undefined && category.newznabCategoryIds === undefined) {
          category.newznabCategoryIds = [category.newznabCategoryId];
          delete category.newznabCategoryId;
          categoryIdsMigrated = true;
        }
      }
      if (categoryIdsMigrated) {
        legacyMigrationNeeded = true;
        logger.info('Migrated legacy nzb category newznabCategoryId field to newznabCategoryIds');
      }
    }
