  ...
  handleStdoutChunk(chunk) {
    chunk
      ...
      .forEach((line) => {
        ...
        // Control marker from the post-download transcode step (see
        // transcodeDownloadedVideo in videoDownloadPostProcessFiles.js) -
        // repeats throughout the encode, unlike VIDEO_PERSISTED_MARKER's
        // one-shot signal. Counts as activity so a long encode doesn't trip
        // DownloadTimeoutController's inactivity timeout (no matching
        // marker line would otherwise reset it during a silent ffmpeg run).
        if (line.startsWith(TRANSCODE_PROGRESS_MARKER)) {
          this.timeoutController.noteActivity();
          try {
            const payload = JSON.parse(line.slice(TRANSCODE_PROGRESS_MARKER.length).trim());
            const structuredProgress = this.monitor.processTranscodeProgress(payload);
            this.emitProgressMessage('Transcoding downloaded video...', structuredProgress);
          } catch (err) {
            logger.warn({ err, line }, 'Failed to parse transcode progress marker');
          }
          return;
        }
