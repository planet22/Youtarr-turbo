...
jest.mock('../../mediaServers', () => ({
  serverRegistry: {
    triggerLibraryScansForNonPlexServers: jest.fn().mockResolvedValue(),
  },
}));
...
const { serverRegistry } = require('../../mediaServers');
...
describe('downloadCompletionEffects', () => {
  ...
  describe('job transition', () => {
    it('refreshes each distinct subfolder once and starts the next job', async () => {
      ...
      expect(serverRegistry.triggerLibraryScansForNonPlexServers).toHaveBeenCalledTimes(1);
    ...
    it('skips Plex refresh when no videos were downloaded but still starts next job', async () => {
      ...
      expect(serverRegistry.triggerLibraryScansForNonPlexServers).not.toHaveBeenCalled();
      expect(jobModule.startNextJob).toHaveBeenCalled();
    });
