...
describe('resolveCommandSettings', () => {
  ...
  test('override beats channel, playlist, and global', () => {
    ...
    expect(out).toEqual({ resolution: '2160', audioFormat: 'video_mp3', skipVideoFolder: false, mediaMode: 'download' });
  });
  ...
  test('channel beats playlist and global', () => {
    ...
    expect(out).toEqual({ resolution: '480', audioFormat: 'mp3_only', skipVideoFolder: true, mediaMode: 'download' });
  });
  ...
  test('playlist beats global when channel is null', () => {
    ...
    expect(out).toEqual({ resolution: '720', audioFormat: 'mp3_only', skipVideoFolder: false, mediaMode: 'download' });
  });
  ...
  test('uses config-provided resolution when nothing else set', () => {
    ...
    expect(out).toEqual({ resolution: '480', audioFormat: null, skipVideoFolder: false, mediaMode: 'download' });
  });
  ...
  test('falls back to DEFAULT_RESOLUTION when config has no resolution', () => {
    ...
    expect(out).toEqual({ resolution: '1080', audioFormat: null, skipVideoFolder: false, mediaMode: 'download' });
  });
...
describe('resolveMediaMode', () => {
  test('override wins over channel, playlist, and global', () => {
    expect(resolver.resolveMediaMode({
      override: { mediaMode: 'both' },
      channel: { media_mode: 'strm' },
      playlist: { media_mode: 'strm' },
      config: { mediaMode: 'download' },
    })).toBe('both');
  });

  test('channel wins over playlist and global', () => {
    expect(resolver.resolveMediaMode({
      override: {},
      channel: { media_mode: 'strm' },
      playlist: { media_mode: 'both' },
      config: { mediaMode: 'download' },
    })).toBe('strm');
  });

  test('playlist wins over global when channel is null', () => {
    expect(resolver.resolveMediaMode({
      override: {},
      channel: null,
      playlist: { media_mode: 'strm' },
  ...
  test('falls back to global config when nothing else set', () => {
    expect(resolver.resolveMediaMode({
      override: {}, channel: null, playlist: {}, config: { mediaMode: 'strm' },
    })).toBe('strm');
  });

  test('defaults to download when config has no mediaMode', () => {
    expect(resolver.resolveMediaMode({ override: {}, channel: null, playlist: {}, config: {} })).toBe('download');
  });

  test('null channel/playlist media_mode falls through instead of short-circuiting', () => {
    expect(resolver.resolveMediaMode({
      override: {},
      channel: { media_mode: null },
      playlist: { media_mode: null },
      config: { mediaMode: 'strm' },
    })).toBe('strm');
  });
});
