...
async function runCompletionSideEffects({
  ...
  // Only refresh media servers and start next job if not processing multiple groups
  if (!skipJobTransition) {
    if (subfoldersInUse.size > 0) {
      ...
      require('../mediaServers').serverRegistry
        .triggerLibraryScansForNonPlexServers(configModule.getConfig(), [...subfoldersInUse])
        .catch(err => {
          logger.error({ err }, 'Failed to trigger Jellyfin/Emby library scan');
        });
    }
