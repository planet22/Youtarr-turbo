...
// STRM cache-on-play jobs, enqueued opportunistically when a STRM item is
// played (see strmCacheOnPlay.js). Always a URL-list job for exactly one
// video; the trailing "[youtubeId]" is a correlation suffix strmCacheOnPlay.js
// matches on to detect an already-in-flight cache job, not just display text.
const STRM_CACHE_LABEL_PREFIX = 'STRM Cache: ';
// Jobs enqueued via the Newznab/SABnzbd emulation (see server/routes/nzb.js)
// when Sonarr/Radarr/Prowlarr grab a YouTube result. Always a URL-list job
// for exactly one video.
const NZB_LABEL_PREFIX = 'Sonarr/Radarr: ';

// Build the activity-view job label for an NZB-triggered download.
function nzbDownloadJobLabel(categoryName, youtubeId) {
  return `${NZB_LABEL_PREFIX}${categoryName} [${youtubeId}]`;
}
...
function isSpecificUrlDownloadJob(jobType) {
  ...
  return (
    ...
    jobType.startsWith(STRM_CACHE_LABEL_PREFIX) ||
    jobType.startsWith(NZB_LABEL_PREFIX)
  );
}
