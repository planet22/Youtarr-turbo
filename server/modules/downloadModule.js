...
class DownloadModule {
  ...
  async doChannelDownloads(jobData = {}, isNextJob = false) {
    ...
    // Fresh invocations (cron tick / manual "run now") only. Queued jobs
    // (isNextJob / jobData.id set) are startNextJob re-invoking this same
    // function to actually execute a job already created on the fresh
    // call — skip all of this so STRM materialization and the "nothing to
    // do" check don't run a second, redundant time for the same logical
    // download run.
    let hadStrmChannels = false;
    if (!isNextJob && !jobData?.id) {
      // STRM-tier channels are handled entirely separately from the batch
      // yt-dlp path below — they materialize .strm files instead of
      // downloading (see channelStrmDownloader). Must run before the
      // "nothing to do" pre-check just below: getEnabledChannelDownloadUrls
      // excludes STRM-tier channels, so a run where every enabled channel
      // happens to be STRM-only would otherwise look empty and skip
      // entirely, never reaching this.
      try {
        const strmChannels = await channelDownloadGrouper.getStrmModeChannels(configModule.config);
        hadStrmChannels = strmChannels.length > 0;
        if (hadStrmChannels) {
          const channelStrmDownloader = require('./channelStrmDownloader');
          await channelStrmDownloader.runStrmChannelDownloads(strmChannels, {
            runId: this.getJobDataValue(jobData, 'runId'),
            videoCount: overrideSettings.videoCount || configModule.config.channelFilesToDownload,
            allowRedownload: !!overrideSettings.allowRedownload,
          });
        }
      } catch (err) {
        logger.error({ err }, 'STRM auto-download sweep failed; continuing with normal channel downloads');
      }

      // Skip gracefully when no enabled channel has any auto-download tab
      // left for the batch path: creating a job that can only fail
      // pollutes Download History and logs a misleading ERROR every cycle
      // on playlist-only (or, now, STRM-only) setups.
      try {
    ...
    try {
      ...
      if (!groups || groups.length === 0) {
        if (hadStrmChannels) {
          // Every enabled channel was STRM-tier and already handled above —
  ...
  async doGroupedChannelDownloads(jobData, groups, isNextJob = false) {
    ...
    // Jellyfin refreshes just the mapped library per subfolder (see
    // jellyfinAdapter.triggerLibraryScan); Emby has no per-subfolder mapping
    // yet and always does a full rescan - see serverRegistry.triggerLibraryScansForNonPlexServers.
    require('./mediaServers').serverRegistry
      .triggerLibraryScansForNonPlexServers(configModule.config, groups.map(g => g.subFolder ?? null))
      .catch(err => {
        logger.error({ err }, 'Failed to trigger Jellyfin/Emby library scan after grouped download');
      });
  ...
  async doSpecificDownloads(reqOrJobData, isNextJob = false) {
    ...
    if (jobModule.getJob(jobId).status === 'In Progress') {
      ...
      const libraryModeFallback = overrideSettings.libraryModeFallback !== undefined ? overrideSettings.libraryModeFallback : null;
      ...
      // --- STRM early exit (optional) ---
      const strmMaterializer = require('./strmMaterializer');
      const mediaMode = strmMaterializer.resolveMediaMode(
        { mediaMode: channelRecord && channelRecord.media_mode },
        overrideSettings
      );
      const libraryMode = downloadSettingsResolver.resolveFinalLibraryMode({
        channelRecord,
        globalDefault: configModule.config.defaultLibraryMode,
      });
      // Same subfolder precedence the real download pipeline uses (hard
      // override > tracked channel's own sub_folder > playlist fallback >
      // global default) - STRM previously only ever looked at a hard
      // override and ignored the channel/global subfolder entirely.
      const strmResolvedSubFolder = downloadSettingsResolver.resolveFinalSubfolder({
        hardOverride: subfolderOverride,
        channelRecord,
        softFallback: subfolderFallback,
      ...
      const strmSubFolder = strmResolvedSubFolder
        || (libraryMode === 'series' ? (configModule.config.seriesOutputSubfolder || '').trim() || null : null);
      ...
      if (mediaMode === 'strm') {
        try {
          ...
          const results = await strmMaterializer.materializeMany(urls, {
            ...
            seriesEpisodeOverride: overrideSettings.seriesEpisodeOverride ?? null,
            // NZB grabs only need the .strm itself - Sonarr/Radarr generate
            // their own artwork/nfo on import, so skip Youtarr's.
            skipMediaSidecarFiles: overrideSettings.skipMediaSidecarFiles === true,
          });
          ...
          // Keep queue moving (same as normal completion paths)
          await jobModule.startNextJob();
          return jobId;
        } catch (err) {
          logger.error({ err, jobId }, 'STRM materialize failed');
          await jobModule.updateJob(jobId, {
            status: 'Error',
            output: err.message || 'STRM materialize failed',
          });
          await jobModule.startNextJob();
          return jobId;
        }
      ...
      this.downloadExecutor.doDownload(
        ...
        false,
        {
          ...
          // STRM cache-on-play: pins the final path to the existing .strm's
          // folder instead of re-resolving it from channel settings. See
          // strmCacheOnPlay.js and videoDownloadPostProcessFiles.js.
          strmCacheTarget: this.getJobDataValue(jobData, 'strmCacheTarget') || null,
          // Real season/episode from an NZB grab's tvsearch (see
          // server/routes/nzb.js), used instead of the upload-year-as-season
          // default in seriesEpisodeResolver.js when the channel/playlist is
          // in TV Series library mode.
          seriesSeasonOverride: overrideSettings.seriesSeasonOverride ?? null,
          seriesEpisodeOverride: overrideSettings.seriesEpisodeOverride ?? null,
          // NZB grabs only need the video file itself - Sonarr/Radarr
          // generate their own artwork/nfo on import, so skip Youtarr's
          // nfo/season.nfo/tvshow.nfo/fanart/backdrop/poster generation.
          skipMediaSidecarFiles: overrideSettings.skipMediaSidecarFiles === true,
        }
      );
    }
  ...
  async doPlaylistDownloads(playlist, options = {}) {
    ...
    for (const group of groups) {
      ...
      if (routing.libraryModeFallback !== undefined) groupOverride.libraryModeFallback = routing.libraryModeFallback;
  ...
  terminateCurrentDownload() {
    ...
    // No live yt-dlp process to SIGTERM - the in-progress job may be a STRM
    // materialize batch instead, which has no child process for
    // downloadExecutor to reach. Cancel it between videos.
    const inProgressJobId = jobModule.getInProgressJobId();
    if (inProgressJobId) {
      const strmMaterializer = require('./strmMaterializer');
      if (strmMaterializer.cancelActiveJob(inProgressJobId)) {
        return inProgressJobId;
      }
    }
