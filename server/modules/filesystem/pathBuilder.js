...
/**
 * Build the path to a TV Series library mode season folder, one level
 * below a channel's root folder.
 * @param {string} channelFolderPath - Full path to the channel's root folder (never season-suffixed)
 * @param {number} season - Season number (calendar year of upload_date)
 * @returns {string} - Full path to the "Season <year>" folder
 */
function buildSeasonFolderPath(channelFolderPath, season) {
  return path.join(channelFolderPath, composeSeasonFolderName(season));
}
