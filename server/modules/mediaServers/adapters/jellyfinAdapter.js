...
class JellyfinAdapter extends BaseAdapter {
  ...
  /**
   * Library sections (Jellyfin's "virtual folders"), for the per-subfolder
   * mapping dropdown - the same role Plex's GET /library/sections plays for
   * plexModule.getLibrariesWithParams.
   */
  async listLibraries() {
    const res = await axios.get(`${this.url}/Library/VirtualFolders`, { headers: this._headers(), timeout: REQUEST_TIMEOUT_MS });
    return (res.data || []).map((f) => ({ id: f.ItemId, title: f.Name }));
  }

  /**
   * Resolves a configured jellyfinSubfolderLibraryMappings entry for
   * `subfolder` (resolved name, no __ prefix; null = root), mirroring
   * plexModule.getLibraryIdForSubfolder.
   */
  _getLibraryIdForSubfolder(subfolder) {
    const mappings = Array.isArray(this.config.jellyfinSubfolderLibraryMappings)
      ? this.config.jellyfinSubfolderLibraryMappings
      : [];
    const normalized = subfolder || null;
    const match = mappings.find((m) => m && (m.subfolder || null) === normalized);
    return (match && match.libraryId) || null;
  }

  /**
   * Refreshes just the library mapped to `subfolder` via POST /Items/{Id}/Refresh
   * when a mapping exists, falling back to the blanket POST /Library/Refresh
   * (every library) otherwise - same fallback shape as before this method took
   * a subfolder, so an unmapped/omitted subfolder behaves identically to the
   * old always-blanket-scan behavior.
   */
  async triggerLibraryScan(subfolder) {
    const libraryId = this._getLibraryIdForSubfolder(subfolder);
    if (libraryId) {
      try {
        ...
        logger.warn({ ...describeHttpError(err), libraryId, subfolder }, 'jellyfin: targeted library refresh failed, falling back to full scan');
        // fall through to the blanket scan below
      }
    }
    try {
