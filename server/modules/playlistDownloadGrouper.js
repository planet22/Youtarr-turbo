...
class PlaylistDownloadGrouper {
  ...
  async buildGroups(playlist, entries, overrideSettings = {}) {
    ...
    for (const entry of entries) {
      ...
      const { resolution, skipVideoFolder, mediaMode } = resolved;

      const key = JSON.stringify({ resolution, audioFormat, skipVideoFolder, mediaMode });
      if (!groups.has(key)) {
        groups.set(key, { resolution, audioFormat, skipVideoFolder, mediaMode, youtubeIds: [] });
      }
