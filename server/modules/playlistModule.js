...
class PlaylistModule {
  ...
  async _fetchPlaylistVideos(playlistId) {
    ...
    // Python regex engine (one subprocess call for the whole batch) - same
    // engine yt-dlp's own --match-filter uses. See pythonTitleMatcher for
    // why this replaced a JS RegExp implementation.
    const titleMatched = playlist.title_filter_regex
      ? new Set(pythonTitleMatcher.filterByTitleRegex(available, playlist.title_filter_regex, (e) => e.title))
      : null;
    const passes = (e) => {
      ...
      if (titleMatched && !titleMatched.has(e)) return false;
      return true;
    };
