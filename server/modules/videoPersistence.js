...
class VideoPersistence {
  ...
  prepareVideoDataForSave(video, isNewVideo = false) {
    ...
    // A metadata-only update (no file found) must never flip is_strm either
    // direction - only a call that actually verified a real file or a real
    // .strm on disk gets to state an opinion on it.
    if (!hasVerifiedFile && !isNewVideo) {
      delete data.is_strm;
    }
  ...
  async upsertVideoForJob(video, jobInstance, alwaysCreateJobVideo = false) {
    ...
    if (videoInstance) {
      // Captured BEFORE the update - the state at the moment this call
      // started. Only a row that WAS genuinely STRM'd is a candidate for
      // sidecar archival below.
      const previousStrmFilePath = (videoInstance.is_strm && videoInstance.filePath)
      ...
      // previously STRM-only - archive (not delete) the old .strm/.strmtool.json
  ...
  /**
   * Archives (renames, never deletes) a stale `.strm` + its `.strmtool.json`
   * sidecar once a real download has landed on a row that was previously
   * STRM-only. Renamed to a `.cached` suffix so Jellyfin's scanner no longer
   * sees them as a second playable item alongside the real file, but
   * `videoDeletionModule.deleteVideoById` can still find and restore them
   * later if automatic removal reclaims the real file's disk space.
  ...
  _archiveStaleStrmSidecars(oldStrmFilePath) {
    ...
    for (const p of [oldStrmFilePath, oldCachePath]) {
      try {
        if (fs.existsSync(p)) {
