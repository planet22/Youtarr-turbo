...
module.exports = function createChannelRoutes({ verifyToken, channelModule, archiveModule, channelDownloadAllModule, ratingMapper }) {
  ...
  /**
   * @swagger
   * /api/channels/{channelId}/combined-filter-preview:
   *   get:
   *     summary: Preview title filter + season/episode decode regex together
   *     description: For channels with both filters set, preview which recent videos the title filter would let download, and the season/episode/filename each would resolve to.
   *     tags: [Channels]
   *     parameters:
   *       - in: path
   *         name: channelId
   ...
   *           type: string
   *       - in: query
   *         name: season_episode_regex
   *         schema:
   *           type: string
   *     responses:
   *       200:
   *         description: Combined preview results
   *       500:
   *         description: Failed to preview combined filters
   */
  router.get('/api/channels/:channelId/combined-filter-preview', verifyToken, async (req, res) => {
    try {
      const { title_filter_regex, season_episode_regex } = req.query;
      const result = await channelSettingsModule.previewCombinedFilters(
        req.params.channelId,
        title_filter_regex || '',
  ...
  router.get('/getchannelvideos/:channelId', verifyToken, async (req, res) => {
    ...
    // Opt-in per-video "would this pass the channel's own Download
