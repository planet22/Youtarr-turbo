...
module.exports = function createConfigRoutes({ verifyToken, configModule, validateEnvAuthCredentials, isWslEnvironment, filenamePreviewRateLimiter }) {
  ...
  /**
   * Generates a new NZB (Sonarr/Radarr/Prowlarr) API key, replacing any
   * existing one. The raw key is returned exactly once - only its SHA-256
   * hash and an 8-char prefix are persisted, mirroring apiKeyModule.js's
   * hashing approach. See server/routes/nzb.js for where it's checked.
   */
  router.post('/api/nzb/regenerate-key', verifyToken, (req, res) => {
