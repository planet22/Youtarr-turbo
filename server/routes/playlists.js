...
function createPlaylistRoutes({ verifyToken, playlistModule, downloadModule, m3uGenerator, mediaServers, models, channelSettingsModule, ratingMapper, subfolderModule, playlistVideoFilters }) {
  ...
  const VALID_MEDIA_MODES = new Set(['download', 'strm', 'both']);
  const VALID_LIBRARY_MODES = new Set(['movie', 'series']);
  ...
  router.put('/api/playlists/:playlistId/settings', verifyToken, async (req, res) => {
    const allowed = ['default_sub_folder', 'video_quality', 'min_duration', 'max_duration', 'title_filter_regex', 'audio_format', 'default_rating', 'sort_order', 'media_mode', 'library_mode'];
    const updates = {};
    for (const k of allowed) if (k in req.body) updates[k] = req.body[k];
    ...
    if ('media_mode' in updates && updates.media_mode !== null && !VALID_MEDIA_MODES.has(updates.media_mode)) {
      return res.status(400).json({ error: 'Invalid media_mode; expected download, strm, both, or null' });
    }
    if ('library_mode' in updates && updates.library_mode !== null && !VALID_LIBRARY_MODES.has(updates.library_mode)) {
      return res.status(400).json({ error: 'Invalid library_mode; expected movie, series, or null' });
    }
    if ('title_filter_regex' in updates) {
      const regexValidation = channelSettingsModule.validateTitleRegex(updates.title_filter_regex);
      if (!regexValidation.valid) {
        return res.status(400).json({ error: regexValidation.error });
      }
    }
    try {
      const p = await findEnabledPlaylist(req.params.playlistId);
