...
function createVideoDetailRoutes({ verifyToken, videoMetadataModule, mediaServers }) {
   ...
   *         description: Invalid YouTube ID
   */
  router.get('/api/videos/:youtubeId/watch-status', verifyToken, async (req, res) => {
    const { youtubeId } = req.params;
    if (!youtubeId || !/^[A-Za-z0-9_-]{6,20}$/.test(youtubeId)) {
      return res.status(400).json({ error: 'Invalid YouTube ID' });
    }
    try {
      ...
      res.status(500).json({ error: 'Failed to retrieve watch status' });
    }
  });
  ...
  router.get('/api/videos/:youtubeId/stream', queryTokenToHeader, verifyToken, async (req, res) => {
    ...
    try {
      ...
      // STRM-only videos: hand off to /api/ytstream so the browser plays a
      // real media stream (direct redirect or ffmpeg pipe) instead of the
      // .strm text file on disk.
      if (streamInfo.isStrm) {
        const cfg = configModule.getConfig();
        const ytCfg = cfg.ytstream || {};
        const mode = ytCfg.defaultMode || 'direct';
        const container = ytCfg.container || 'mp4';
        const quality = String(
          ytCfg.quality || cfg.preferredResolution || '720'
        );
        let transcode = ytCfg.transcode;
        if (!transcode) {
          const codec = cfg.videoCodec;
          transcode = codec === 'h264' || codec === 'h265' ? 'h264' : 'copy';
        }
        const params = new URLSearchParams({ mode, quality, container, transcode });
        const target = `/api/ytstream/${encodeURIComponent(youtubeId)}?${params.toString()}`;
        req.log.info({ youtubeId, target }, 'STRM video: redirecting in-app stream to ytstream');
