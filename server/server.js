...
const initialize = async () => {
  try {
    ...
    const apiLimiter = rateLimit({
      ...
      keyGenerator: (req) => getRateLimitAddress(req),
      // STRM playback (HLS segments especially) can legitimately need far
      // more than 100 requests/minute from a single media-server IP while
      // buffering. Those requests are already scoped to validated video
      // IDs / an upstream-host allowlist, so they don't need this general
      // per-IP throttle on top.
      skip: (req) => req.path.startsWith('/ytstream/'),
    });
